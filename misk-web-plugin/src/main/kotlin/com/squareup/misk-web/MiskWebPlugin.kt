...
open class MiskWebPlugin : Plugin<Project> {
  ...
  fun dockerBuildContainer(
    ...
    val imageVersion =
        when (simpleFlatJsonToMap("${project.projectDir}${relPath}").get("version")) {
          "latest" -> "0.1.3"
          "alpha" -> "0.1.3"
          null -> "0.1.3"
          else -> simpleFlatJsonToMap("${project.projectDir}${relPath}").get("version")
        }
    val command =
  ...
  fun dockerInformation(task: String, dockerName: String, dockerCmd: String): String {
    return """
    |  Task: ${task}
      ...
      tasks {
        register("webBuild", Task::class) {
          ...
          packagesPaths.forEach {
            ...
            println(dockerInformation("webBuild::packages", dockerName, dockerCmd))
          ...
          tabPaths.forEach {
            ...
            println(dockerInformation("webBuild::tabs", dockerName, dockerCmd))
            println(dockerCmd.runCommand())
          }
        }

        register("webStart", Task::class) {
          println("com.squareup.misk-web-plugin::webStart")
          val tabPaths = recursiveFind(project, "/web/tabs/", 3)
          tabPaths.forEach {
            val dockerName = dockerBuildContainerName(it)
            val dockerCmd = dockerBuildContainer(project, it, name = dockerName,
                runtime = "/bin/misk-web -n -d")
            println(dockerInformation("webStart", dockerName, dockerCmd))
            println(dockerCmd.runCommand())
          }
        }
      }
